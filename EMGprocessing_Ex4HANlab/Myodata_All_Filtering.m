%----------------------------------------------------------\
% Myodata_All_Filtering.m
% Author : Han U. Yoon (2021/02/21)
% Purpose: To give some nice exprience to my commrade J.H. and H.J.
% Revise note: 
%----------------------------------------------------------\

function Myodata_All_Filtering(instrExercise,innumTrial,strcmap, strAAFTnum)
...
%- Normalize sigDATA
sigDATAnormal1 = retNormalizedData(sigDATA1,numEMG);  
sigDATAnormal2 = retNormalizedDataEachCh(sigDATA2,numJANGLE);  
...
switch strAAFTnum
    case {'AAFT(0)','AAFT(2)','AAFT(3)','AAFT(4)','AAFT(5)','AAFT(10)','AAFT(20)'}
        filename2 = sprintf('./resultFilteredSig_%s/%s/%s_C%s_trial(%d).txt',strAAFTnum,strExercise,strExercise,strcmap, numTrial);
    otherwise
        error('Wrong AAFT options');
end

% fid2 = fopen(filename2,'w');
% [m,n] = size(sigDATAcombined);
% tempDATA = reshape(sigDATAcombined, [1, (m*n)]);
tempDATA = sigDATAnormal;
% [m,n] = size(tempDATA)
